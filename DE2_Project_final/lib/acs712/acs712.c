...
float acs712_read_current(void) {
    ...
    for (uint8_t i = 0; i < 100; i++) {
        ...
        float voltage = (adc_value / 1023.0) * acs712_ref_voltage; // Convert to voltage in volts
        float current = (voltage - acs712_offset) / acs712_sensitivity; // Convert to current in amperes
